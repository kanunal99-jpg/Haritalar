...
object MapLayerControlBridge {
    ...
    private fun addControls(activity: Activity, root: ViewGroup, map: MapLibreMap) {
        ...
        val container = LinearLayout(activity).apply {
            tag = CONTROL_TAG; orientation = LinearLayout.VERTICAL; gravity = Gravity.CENTER
            setPadding((6 * density).toInt(), (6 * density).toInt(), (6 * density).toInt(), (6 * density).toInt())
            background = rounded(0xF5FFFFFF.toInt(), 18f * density); elevation = 8f * density
        }
        val button = Button(activity).apply {
            text = "Katmanlar"; setAllCaps(false); textSize = 12f; minHeight = (46 * density).toInt()
    ...
    private fun togglePanel(activity: Activity, root: ViewGroup, map: MapLibreMap) {
        ...
        val panel = LinearLayout(activity).apply {
            tag = PANEL_TAG; orientation = LinearLayout.VERTICAL
            setPadding((10 * density).toInt(), (10 * density).toInt(), (10 * density).toInt(), (10 * density).toInt())
            background = rounded(0xF8FFFFFF.toInt(), 20f * density); elevation = 10f * density
        }
        ...
        val camera = Button(activity).apply {
            setAllCaps(false); textSize = 12f; refreshCameraLabel(this, map)
            setOnClickListener { val current = map.cameraPosition; val tilt = if (current.tilt >= 30.0) 0.0 else 45.0; map.cameraPosition = CameraPosition.Builder(current).tilt(tilt).build(); refreshCameraLabel(this, map) }
        }
    ...
    private fun makeLayerToggle(activity: Activity, title: String, map: MapLibreMap, layerIds: List<String>): Button = Button(activity).apply {
        ...
        fun refresh() {
            val state = groupVisibility(map, layerIds); isEnabled = state != null
            text = when (state) { true -> "$title: Açık"; false -> "$title: Kapalı"; null -> "$title: Kullanılamıyor" }
        }
        refresh()
        setOnClickListener { val current = groupVisibility(map, layerIds) ?: run { refresh(); return@setOnClickListener }; setVisible(map, layerIds, !current); refresh() }
    }
    ...
    private fun trafficLayerIds(map: MapLibreMap): List<String> = buildList { add(TOMTOM_RASTER_TRAFFIC_LAYER_ID); TRAFFIC_SEVERITY_LAYER_IDS.forEach { add(TRAFFIC_LAYER_PREFIX + it) } }.filter { map.style?.getLayer(it) != null }
